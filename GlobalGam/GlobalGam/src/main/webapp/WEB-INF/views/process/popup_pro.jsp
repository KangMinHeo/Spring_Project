...
$(document).ready(function(){
	...
	$("#table_check").on("click", "tbody tr[role='row']", function(e){
		...
		$('.re_pro_no_4',opener.document).val($(this).find(".m_no").text());
		$('.pro_name_4',opener.document).val($(this).find(".m_name").text());
			...
			<table id="table_check" class="table_pop">
				...
				<tbody>
					<c:forEach var="met" items="${requestScope.mlist }">
						<tr>
							<td class="m_no">${met.pro_no}</td>
							<td class="m_name">${met.pro_name }</td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
